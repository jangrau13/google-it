...
function getResults(_ref) {
  ...
  var jan = $('#result-stats').text();
  var fa = $('#topstuff').html();
  var approximateResults = ((resultStats.split(' results') || [''])[0].split('About ')[1] || '').replace(',', ''); //logIt(approximateResults)
...
function googleIt(config) {
  ...
  return new Promise(function (resolve, reject) {
    getResponse(config).then(function (_ref3) {
      ...
      var try1 = _objectSpread(_objectSpread({}, stats), results);
